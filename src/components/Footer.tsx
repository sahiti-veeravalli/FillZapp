...
const Footer = () => {
  ...
  const brandScale = useTransform(scrollYProgress, [0.3, 0.8], [0.3, 1]);
  const brandOpacity = useTransform(scrollYProgress, [0.3, 0.7], [0, 0.1]);
  const brandLetterSpacing = useTransform(scrollYProgress, [0.3, 0.8], ["0.5em", "-0.05em"]);
  ...
  return (
    <footer ref={footerRef} className="bg-primary text-primary-foreground relative overflow-hidden">
      ...
      {/* Big brand text with scroll-driven expansion */}
      <div className="section-container py-12 text-center overflow-hidden">
        <motion.h2
          ...
          className="font-display font-extrabold text-6xl sm:text-8xl lg:text-[10rem] tracking-tighter text-primary-foreground select-none leading-none"
        >
          FILLZAPP
        </motion.h2>
      </div>
